...
def test_row_count(data: pd.DataFrame):
    """
    Tests that dataframe is between 15,001 and 999,999 rows
    
    Args:
        data: Current dataset to test
        
    Raises:
        AssertionError: If row count exceeds the threshold
    """
    assert 15000 < data.shape[0] < 1000000

########################################################
# Implement here test_price_range                      #
########################################################

def test_price_range(data: pd.DataFrame, min_price: float, max_price:float):
    """
    Tests the price range of rows in data
    ...
    Raises:
        AssertionError: If prices exceed the threshold
    """
    assert (data['price'].between(min_price, max_price, inclusive='both') == True).all()